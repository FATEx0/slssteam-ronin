...
// contract file the Tsuki host reads.
//
...
// the port alone and Tsuki falls back to 8080 too; both share the endpoint.
...
namespace CefPort
{
	...
	// up listening on, and Tsuki would connect to a dead port.
	...
	// Contract file shared with Tsuki:
	// $HOME/.local/share/Tsuki/cef_port  ("" if HOME is unset).
	inline std::string contractPath()
	{
		...
		return std::string(home) + "/.local/share/Tsuki/cef_port";
	}
	...
	// leave it on the default and let Tsuki fall back to 8080 too (verified on
	// a Bazzite box: multiple CDP clients coexist on one CEF target).
	//
	...
	// Best-effort removal of the Tsuki contract file. Called when we decide to
	// keep CEF on 8080 (Decky coexistence): a STALE contract left by a previous
	// ephemeral-port session would otherwise make Tsuki connect to a dead port
