...
{
	uint64_t parsed = 0;
	CHECK(ManifestPins::parseDecimalGid("18446744073709551615", parsed)
	      && parsed == UINT64_MAX,
	      "gid parser accepts complete uint64 range");
	CHECK(!ManifestPins::parseDecimalGid("", parsed),
	      "gid parser rejects empty input");
	CHECK(!ManifestPins::parseDecimalGid("-1", parsed),
	      "gid parser rejects signs");
	CHECK(!ManifestPins::parseDecimalGid(" 1", parsed),
	      "gid parser rejects whitespace");
	CHECK(!ManifestPins::parseDecimalGid("12x", parsed),
	      "gid parser rejects suffixes");
	CHECK(!ManifestPins::parseDecimalGid("18446744073709551616", parsed),
	      "gid parser rejects overflow");
